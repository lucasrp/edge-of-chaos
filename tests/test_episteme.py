...
class ValencedEdgesProjectWithTheLeadCeiling(unittest.TestCase):
    ...
    def test_report_artifact_projects_curated_experiment_metadata_for_the_ui(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(params["caveat"], "tested on one report family")
            self.assertEqual(params["supports"], ["arm C beat arm A"])
            self.assertEqual(params["excludes"], ["no-gate baseline"])
            self.assertEqual(params["next"], "ship as default")
            self.assertEqual(params["report_slug"], "report-exp40")
