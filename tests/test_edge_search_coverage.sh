...
echo "--- Test 1: required groups covered when topic/memory both hit ---"
...
mod.search_with_sidecar = lambda query, limit=10, doc_type=None, conn=None: ([], [])
mod.embed_text = lambda query: [0.1, 0.2, 0.3]

results, coverage, sidecar = mod.search_with_coverage(
    "heartbeat timeout",
    limit=5,
    required_types=["topic", "memory"],
    conn=object(),
)

assert coverage["required_covered"] is True
assert coverage["missing_required_types"] == []
assert [item["name"] for item in coverage["required"]] == ["topic", "memory"]
assert [item["hit_count"] for item in coverage["required"]] == [1, 1]
assert [item["type"] for item in results[:2]] == ["topic", "note"]
assert sidecar == []
PY
then
...
assert [item["covered"] for item in coverage["required"]] == [True, False]
