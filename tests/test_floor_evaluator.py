...
class RegistryAndFailLoud(unittest.TestCase):
    ...
    def test_visual_producers_owe_their_form_floor(self):
        # R1 (S6) — FLIPPED from the old ascii-fallback expectation. The floor demands the RENDERABLE
        # diagram/chart; `ascii-diagram` is DROPPED (S7) and does NOT satisfy it. The visual floor is
        # CAPABILITY-CONDITIONAL: with vl-convert ABSENT it degrades to NOT-OWED (env-skipped, never an
        # ascii substitute).
        PRESENT, ABSENT = {pd.VL_CONVERT: True}, {pd.VL_CONVERT: False}
        # map owes >=2 RENDERABLE illustrations (with the backend present).
        self.assertTrue(pd.presentation_violations({"skill": "map", "content": _content("diagram")}, PRESENT))
        self.assertEqual(pd.presentation_violations(
            {"skill": "map", "content": _content("diagram", "diagram")}, PRESENT), [])
        # ascii does NOT satisfy the renderable floor anymore (the flip).
        self.assertTrue(pd.presentation_violations(
            {"skill": "map", "content": _content("ascii-diagram", "ascii-diagram")}, PRESENT))
        # backend ABSENT → the visual floor is not owed (degrades to grounded prose, not ascii).
        self.assertEqual(pd.presentation_violations({"skill": "map", "content": _content("paragraph")}, ABSENT), [])
        # plan owes framed steps (non-visual, always) + a RENDERABLE illustration (capability-conditional).
        self.assertTrue(pd.presentation_violations({"skill": "plan", "content": _content("paragraph")}, PRESENT))
        self.assertEqual(pd.presentation_violations(
            {"skill": "plan", "content": _content("next-steps-grid", "diagram")}, PRESENT), [])
        self.assertTrue(pd.presentation_violations(   # ascii no longer satisfies the illustration
            {"skill": "plan", "content": _content("next-steps-grid", "ascii-diagram")}, PRESENT))
        # plan framed_steps is owed even wheel-less (non-visual); the illustration env-skips.
        self.assertEqual(pd.presentation_violations(
            {"skill": "plan", "content": _content("next-steps-grid")}, ABSENT), [])
        # discovery owes contextual framing (non-visual).
        self.assertTrue(pd.presentation_violations({"skill": "discovery", "content": _content("paragraph")}, PRESENT))
        self.assertEqual(pd.presentation_violations({"skill": "discovery", "content": _content("callout")}, PRESENT), [])
