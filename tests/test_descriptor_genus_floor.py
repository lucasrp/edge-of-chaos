...
def _diag():
    # a renderable illustration (ascii-diagram was dropped in S7); identity-stable per call site.
    return {"type": "diagram", "layout": "dag",
...
class DescriptorAddsNeverSubtracts(unittest.TestCase):
    def test_developed_prose_map_missing_moves_still_fails_genus(self):
        ...
        # map's presentation floor (>=2 illustrations) is fully satisfied (2 renderable diagrams, with
        # vl-convert mocked present so the capability floor is owned not env-skipped).
        with mock.patch.object(render, "diagram_renderable", return_value=True), \
             mock.patch.object(render, "diagram_available", return_value=True):
    ...
    def test_map_short_on_illustrations_flags_presentation(self):
        # one illustration only; prose present so R0 is clean and the (capability-owned) floor surfaces.
        with mock.patch.object(render, "diagram_renderable", return_value=True), \
             mock.patch.object(render, "diagram_available", return_value=True):
            v = close.check_genus(_art("map", [_para("prose framing the single diagram"), _diag()]))
...
class GatesAgreeWithRendering(unittest.TestCase):
    ...
    def test_production_floor_rejects_unrenderable_diagram(self):
        ...
        para = _para("prose framing the diagrams so R0 is clean and the floor surfaces")
        # vl-convert mocked PRESENT (the comment's "even when installed") so the floor is owned; the
        # per-block renderable check is then the sole authority over whether each diagram counts.
        with mock.patch.object(render, "diagram_renderable", side_effect=lambda b: b is good), \
             mock.patch.object(render, "diagram_available", return_value=True):
            v_bad = close.check_genus(_art("map", [para, bad1, bad2]))
            v_ok = close.check_genus(_art("map", [para, good, good]))
