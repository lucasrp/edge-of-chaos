...
grep -q "Do not close by listing candidate" "$RESEARCH"
grep -q "Bare Invocation / Missing Topic" "$RESEARCH"
grep -q "A bare \`/ed-research\` dispatch is already authorization" "$RESEARCH"
grep -q "Select the highest-leverage target" "$RESEARCH"
grep -q "Produce and publish the research artifact" "$RESEARCH"
grep -q "Ask the operator for a target only when the runtime frame contains no reasonable" "$RESEARCH"
