...
class AcceptedRiskTags(unittest.TestCase):
    """Reviewer risks are non-blocking. The author can resolve some with grounding and explicitly
    keep others as accepted risk tags in the final artefato."""

    def _risk_reviewer(self, identity):
        def r(artefato, complete_fn=None):
            return {
                "pass": True,
                "scores": {"rigor": 4},
                "rationales": {"rigor": "risk marked, not a hard fact defect"},
                "strikes": [],
                "risks": [{
                    "tag": "speculative_architecture",
    ...
    def test_author_can_accept_interpretive_risk_with_tag(self):
        captured = {}
        art = {
            **_conformant_artefato(),
            "accepted_risks": [{
                "tag": "speculative_architecture",
                "claim": "the old Edge returned",
                "rationale": "strong mentor interpretation, not a closed fact",
        ...
        titles = [s.get("title") for s in
                  captured["art"]["content"].get("additional_sections", [])]
        self.assertIn("Riscos autorais marcados", titles)
        rendered = json.dumps(captured["art"]["content"], ensure_ascii=False)
        self.assertIn("speculative_architecture", rendered)
        self.assertIn(close.FEYNMAN_REVIEWER_ID, rendered)
        close.verify_proof(
            captured["proof"], slug=captured["art"]["slug"], spec=captured["art"]["content"],
            intent=captured["art"]["intent"], cites=captured["art"]["cites"],
            proposes=captured["art"]["proposes"], distills=captured["art"]["distills"],
            accepted_risks=result["accepted_risks"])
        tampered = {**captured["proof"], "accepted_risks": [{
            **result["accepted_risks"][0],
            "claim": "a different risk claim",
        }]}
        with self.assertRaises(ValueError):
            close.verify_proof(
                tampered, slug=captured["art"]["slug"], spec=captured["art"]["content"],
                intent=captured["art"]["intent"], cites=captured["art"]["cites"],
                proposes=captured["art"]["proposes"], distills=captured["art"]["distills"],
                accepted_risks=tampered["accepted_risks"])

    def test_unaccepted_reviewer_risk_does_not_publish_a_tag(self):
        captured = {}
        result = close.run_close(
            _conformant_artefato(), produce_fn=_conformant_artefato,
            reviewers=(self._risk_reviewer(close.FEYNMAN_REVIEWER_ID),
    ...
    def test_hard_strikes_still_block_even_when_author_accepts_risk(self):
        ...
        result = close.run_close(
            art, produce_fn=lambda: art,
            reviewers=_both_striking("unsupported date is a hard fact defect"),
            complete_fn=lambda *a, **k: "",
            publish_fn=lambda a, p: published.append(a))
