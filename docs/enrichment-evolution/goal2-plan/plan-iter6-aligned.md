...
- **S2a (congelar benchmark de STORYTELLING, não claim-set global):** rodar o livro `eeb696e` pelo
  extrator; congelar **só** prosa/conceito-ratio, cobertura-de-transição, K-through-line (R0-II). O
  claim-set do livro é congelado **apenas** para a slice de migração do livro-roberto — **não** para o
  gate genérico (sem superset global, CONTEXT/ADR-0013).
...
- **Vermelho:** o report `novoformato` (rico-mas-raso) **falha**; o livro `eeb696e` **passa**; um
  Artefato que define acrônimos mas perde claims/continuidade **da própria fonte** **falha**; um map/plan
  sobre material **não-roberto NÃO falha** por não conter claims do livro (content-relative).
