...
- `add_visuals` (no produtor, evidência viva) emite um **digest sobre {dados-do-bloco + span-de-evidência}**
  ligado ao **proof do close** (o digest que o close já assina); o publish **re-verifica** o digest —
  **proof-only**, nenhuma evidência/contexto do produtor cruza a fronteira de publish.
...
Testes vermelhos: trace **forjado/stale/mismatched** falha; visual **opcional não-sustentado** falha;
**fake-decorativo com dado** falha. (Fecha Q3/F2/F-pub.)
...
- **Arquivos:** `tools/close.py` (`IMPROVE_ROUNDS`→ **detector de ganho-marginal→0 + backstop generoso**
  ...
  `tools/conductor.py`.
- **Vermelho (contrato R7 de 2 partes + severidade):** (a) loop que **platô** (rodada sem mudança de
  substância) **para** e publica com residual; (b) loop que **ainda melhora substancialmente CONTINUA**
  (não é cortado por um N); (c) loop **divergente** para no **backstop**; (d) finding **bloqueante**
  (corretude/evidência/grounding/render-fail **E R0-class**: conceito perdido / visual-substitui-prosa /
  storytelling fino) **falha-fechado, nunca residual**; só não-bloqueante vira residual. **Primeiro
  slice.** (Semantic-discharge conductor-específico em S9; a classificação nasce aqui.)
...
- **S2b (avaliador):** R0-I (inventário de termos incl. labels visuais) + R0-II (continuidade
  ...
  retido**. Wire no `close.check_genus` content-relative.
- **Vermelho R0-III (espelha requirements):** (i) **menção de superfície** que cita o termo mas não
  preserva o claim → FALHA; (ii) **claim contraditório** retido → FALHA; (iii) **paráfrase válida** →
  PASSA; (iv) map/plan não-roberto não false-falha por não conter claims do livro.
