...
- **(a) Strip-and-reinsert real:** o publish **descarta TODO visual owed autorado** e re-insere só via
  `add_visuals` a partir da evidência — o publisher nunca confia no visual do produtor. Exige a
  evidência no seam de publish.
...
- **Severidade inclui R0-class DESDE O DIA 1 (gate Goal2-v2):** findings de corretude/evidência/
  ...
  completude < livro) são **bloqueantes** — nunca viram residual, **mesmo antes de S2** cravar a métrica
  mecânica (até lá, honrados como blocker levantado-pelo-revisor). Só não-bloqueante (estilo, nit) vira
...
### S3a — Contrato do store imutável de run (decisão bloqueante, P2)
- **Objetivo:** decidir o store referenciável para internal-evidence: reusar eventlog/artefato-de-origem
  vs. novo store content-addressed. Entregar a fixture de artefato-de-run imutável.
- **Aceitação:** existe um ref dereferenciável a um run imutável com valores; fixture pronta. **Bloqueia S3.**
...
- **Vermelho:** número com `run_id`/ref **dereferenciável ao store de S3a** e **valor que bate** passa
  sem cite externo; ref que **não resolve** OU **valor divergente** **falha**; sem proveniência falha.
...
- **S2a (congelar benchmark):** rodar o livro `eeb696e` (`roteiro-paper-v8`) pelo extrator de métricas;
  congelar prosa/conceito, cobertura-de-transição, K-through-line, e o **conjunto de conceitos/claims**.
- **S2b (avaliador):** implementar as duas partes (inventário de termos completo incl. labels visuais +
  continuidade) **e a parte (III) completude de conteúdo** (conceitos do enriquecido ⊇ livro); wire no
  `close.check_genus` como gate dominante.
- **Vermelho:** o report `novoformato` (rico-mas-raso) **falha**; o livro `eeb696e` **passa**; um
  Artefato que define acrônimos mas perde conceitos/continuidade **falha**.
...
### S7 — Boundary de grounding de todo visual owed (R2/R3) — **ANTES do floor**
...
- **Vermelho:** visual com proof **forjado/stale/mismatched** (incl. raw-html-SVG) é strippado no
  publish; ascii-edge não sustentado falha mesmo com vl-convert ausente; ascii-edge fundamentado passa.
