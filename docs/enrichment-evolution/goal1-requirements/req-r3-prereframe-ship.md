...
- **R0 — Piso de storytelling: EXPLICAR, não rotular (DOMINANTE, bloqueante).** O formato enriquecido
  ...
  **(I) Inventário de termos COMPLETO** — não só ALL-CAPS: headings, phase-ids, **labels de bloco
  ...
  (mesma seção, em prosa, não em label). **(II) Continuidade narrativa vs. benchmark CONGELADO** do
  livro `eeb696e` — métricas com fórmula definida e **valores numéricos CONGELADOS do benchmark no
  início do Goal 2** (não recalculáveis por rodada, fecha o goalpost-móvel): (a) **prosa-explicativa/
  conceito** = tokens de prosa não-rotular ÷ nº de conceitos do inventário (I), ≥ a razão do livro;
  (b) **cobertura de transição** = fração de fronteiras de seção com ≥1 frase de prosa conectiva, ≥ a do
  livro; (c) **through-line** = a tese é enunciada na abertura e referenciada em ≥K seções (K fixado do
  livro); (d) **um parágrafo explicativo não-visual por estrutura visual/rotulada**. Um Artefato que
  define os acrônimos mas pontua **abaixo dos valores congelados** em (II) **FALHA** — fecha o buraco do
  glossário-decorativo. *(As fórmulas são requisito; os números exatos são congelados no Goal 2 a partir
  de `eeb696e` — gate v-reframe-2 F1.)*
  **(III) Completude de conteúdo (o NORTE):** o Artefato enriquecido cobre **≥ a informação do livro**
  — todo conceito/claim/passo do benchmark está presente; **nenhum conteúdo é trocado por visual**. O
  visual entra como camada adicional sobre a prosa completa, nunca no lugar dela. Aceitação: o conjunto
  de conceitos/claims do enriquecido ⊇ o do livro `eeb696e`; um Artefato que ficou mais visual **e**
  perdeu conceitos/claims **FALHA** (regressão), por mais bonito que esteja.
...
- **R8 — Tier "internal-evidence" no gate de cite/grounding (bloqueante p/ Artefato de medição).** Dado
  ...
  **proveniência INTERNA** (run_id, ref ao artefato de origem) em vez de cite externo. O revisor de cite
  e o move external-frame do rich-rite aceitam internal-evidence para Artefatos de medição interna. Sem
  isto o gate é um **alvo que recua**: o número ou vira texto-de-cite (absurdo: o cite vira o relatório)
  ou some (esvazia o relatório) — a causa-raiz nº1 do livelock do roberto, evidenciada na rodada em que
  o revisor aceitou números da auditoria e então cobrou os de E1/DS2/E2.4 (em outros artefatos).
  *Porquê:* objetivo nº2 — o gate foi calibrado p/ síntese-sobre-o-mundo; a próxima geração produz
  medição interna, que o gate hoje torna impassável. Achado sobre o RITO (vale issue no edge).
  *Aceitação (não basta auto-atestar — gate v-reframe-2 F2):* o ref interno deve ser
  **dereferenciável** a um artefato de run **imutável/content-addressed**, e o valor reportado deve
  **bater** com o valor no artefato referenciado (checado mecanicamente). Um número com ref que **não
  resolve** ou cujo **valor diverge** da fonte **falha** (mesma laundering-guard de `attributable`,
  agora sobre proveniência interna); número sem proveniência alguma **falha**; o tier é explícito — não
  afrouxa o gate de síntese-sobre-o-mundo.
...
- **R2 — Grounding como invariante verificável no publish, não como contrato-de-origem (bloqueante).**
  Invariante: **nenhum visual `owed` reader-visível autorado** (chart, diagram, ascii-diagram fallback,
  **e qualquer alias desenhado — raw-html/svg/custom-html inline-SVG**) chega ao Artefato publicado sem
  que cada datum/aresta resolva contra um span da evidência (`visuals.attributable` + laundering guard).
  Visual desenhado que não pode passar pelo seam (ex.: SVG arbitrário em `raw-html`) é **banido** como
  caminho de visual-autorado-do-produtor — não há caminho de visual que escape o grounding (gate v4, F1). Mandar "produza via `add_visuals`" não basta — o close só vê o spec final e **não
  ...
  Goal 2 entre: (a) **inserção centralizada** — o publisher *strippa* qualquer visual owed pré-desenhado
  e só `add_visuals` (re)insere, grounded por construção; ou (b) **proveniência no proof do close** —
  ...
  *Aceitação:* um chart/diagram/ascii-edge **ou um `raw-html`/SVG-inline** desenhado **direto** no spec
  do produtor (fora do seam) é **rejeitado pelo MESMO runtime que publica**; um visual com edge-list
  atribuída passa; um datum fora da evidência é bounced. Nenhum schema novo de bloco.
