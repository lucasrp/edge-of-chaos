...
- **R2 — Grounding como invariante verificável no publish, não como contrato-de-origem (bloqueante).**
  ...
  *Aceitação:* (a) um chart/diagram/ascii-edge **ou `raw-html`/SVG-inline** desenhado **direto** no spec
  ...
  (não-owed)** com dado não sustentado é **rejeitado** (fecha o bypass do opcional); (c) um visual
  **explicitamente decorativo (não-claim)** passa pela regra estreita **mas falha se carregar
  dado/relação**; (d) um visual com edge-list atribuída passa; um datum fora da evidência é bounced.
