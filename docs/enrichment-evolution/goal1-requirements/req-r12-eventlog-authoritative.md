...
- **R2 — Grounding como invariante verificável no publish, não como contrato-de-origem (bloqueante).**
  ...
  com a evidência viva) emite um **digest sobre {dados-do-bloco + span-de-evidência}** ligado ao digest
  que o close já assina; o publish **re-verifica** o digest e strippa o que não bate. **Strip-and-reinsert
  no publish está FORA** (exigiria evidência/contexto no publish, violando o blindfold do publisher —
  ADR-0013: publish é blind, final-Artefato-only). `add_visuals` retorna **shortfall** quando a
  evidência não sustenta, nunca fabrica. *(Reusa `visuals.py`; resolve Q3; proof-only respeita ADR-0013.)*
