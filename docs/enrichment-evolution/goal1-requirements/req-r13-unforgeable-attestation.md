...
- **Q3. (RESOLVIDA — proof-only.)** O mecanismo é o **proof-digest por-visual** ligado ao proof do
  close, re-verificado no publish (proof-only; nenhuma evidência/contexto do produtor cruza — ADR-0013
  blind publisher). **Inserção centralizada está FORA** (rotearia evidência ao publish). A fiação
  produtor→`add_visuals` (estender do conductor aos standalone) é necessária; o publish nunca recebe
  contexto do produtor, só re-verifica o digest. Red tests: forjado/stale/mismatched, opcional
  não-sustentado, fake-decorativo-com-dado.
