...
- **R0 — Piso de storytelling: EXPLICAR, não rotular (DOMINANTE, bloqueante).** O formato enriquecido
  ...
  **(III) Completude de conteúdo (o NORTE) — contrato de inventário de claims CONGELADO (gate
  reframe-3 F1):** o benchmark `eeb696e` é destilado num **inventário de claims tipados** — cada claim
  com `claim-id`, **span de origem**, entidades normalizadas — congelado no Goal 2. Cada claim do
  benchmark deve estar **suportado em prosa não-visual** no enriquecido, casado por **regras explícitas**
  (paráfrase / split / merge), **sem claim retido contraditório**. Visual é camada adicional sobre a
  prosa completa, nunca no lugar dela. Testes negativos obrigatórios: (a) **menção de superfície** que
  cita o termo mas **não preserva o claim** → FALHA; (b) **claim contraditório** retido → FALHA; (c)
  **paráfrase válida** do claim → PASSA. Isto tira a completude do juízo subjetivo (resolve a P3).
...
- **R2 — Grounding como invariante verificável no publish, não como contrato-de-origem (bloqueante).**
  ...
  uma fuga. Só visual **explicitamente decorativo (não-claim)** passa por uma regra estreita à parte.
