...
class _VisibleTextParser(_HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        is_hidden = _attrs_mark_hidden(attrs)
    ...
    def handle_startendtag(self, tag, attrs):  # e.g. `<br/>` — void, no matching end tag
        if not _attrs_mark_hidden(attrs):
            self._maybe_break(tag)
...
# DOCUMENTED FOLLOW-ON (operator decision, S7): the STRUCTURED data visuals (metrics-grid / comparison /
# comparison-table) are NOT hard-grounded here. Their fabricated-data risk is real but (a) token-matching
# authored values against paraphrased cite snippets is fragile (false-positives on legit data), and
# (b) the conductor's writers emit them with NO grounding seam, so a hard guard cascades into the
# conductor's own genus check. They are governed by R0 (labels explained in prose) + R8 (prose numeric
# claims grounded via the runstore) + visual-coverage; a dedicated structured-visual attestation needs a
# conductor grounding seam for them + the richer explorer-evidence pipeline (P1) — tracked as R2-structured.
...
def _render_visible(html_str) -> str:
    """Visible text of one rendered HTML fragment: drops any hidden/styled subtree (the boolean `hidden`
    attr / aria-hidden / ANY inline `style` — author CSS can hide via opacity:0 / font-size:0 / offscreen /
    display:none / …), so styled-invisible content contributes nothing (Codex S3 #4/#5/#6). '' on error."""
...
def _style_hides_text(safe: str) -> bool:
    """True iff the SANITIZED inline style hides/collapses the text (ALLOWLIST model): parsed
    declaration-by-declaration with exact property names. A declaration hides if it is a known hide-capable
    property with a hiding value (display:none; visibility:hidden|collapse; opacity/font-size signed-zero;
    scale zero-axis; `font:` shorthand zero size; transparent/zero-alpha color; large-negative text-indent),
    OR its property is NOT in `_R0_SAFE_PROSE_PROPS` (an unknown/exotic property → fail-closed). Returns
    False only when EVERY declaration is a known-safe, non-hiding one."""
    for decl in safe.split(";"):
        ...
        if prop == "text-indent" and _text_indent_hides(val):
            return True
        if prop not in _R0_SAFE_PROSE_PROPS:
