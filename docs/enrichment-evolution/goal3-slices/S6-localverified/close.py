...
# (chart data, diagram nodes+edges) that must each resolve to an evidence span. Structured palette blocks
# (table/metrics-grid/comparison/…) are NOT in scope here — they are R0/visual-coverage concerns, not
# free-data grounding. Only `chart`/`diagram` (the renderable recipes whose data is structurally
# attributable) are GROUNDABLE; every other drawn/authored-visual path — `raw-html`/`svg`/`html`/
# `custom-html` (too free-form) AND `ascii-diagram` (operator decision: free-form ascii relations can't be
# soundly grounded and carry no advantage over the renderable recipes) — is an UNGROUNDABLE authored visual
# and rejected. A groundable visual must carry a valid attestation minted by `ground_visuals`.
