...
def _form_owes_visual(skill) -> bool:
    """True iff the producer's DESCRIPTOR declares a visual-form floor — a `require` rule (incl. nested
    `any_of` options) naming a visual block type."""
    def _types(rule):
    ...
    return bool(declared & _VISUAL_REQUIRE_TYPES)
