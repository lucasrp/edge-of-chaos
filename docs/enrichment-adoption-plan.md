...
### S3 — Tier internal-evidence (R8) — depende de S3a  ✅ SHIPPED (codex approve, iter-10)
- **Entregue em `tools/close.py`:** o tier internal-evidence no gate de cite/grounding (`_check_cites`),
  com o corpus de claim = **prosa reader-visible** (`_prose_blocks`→`PROSE_BLOCK_TYPES` compartilhado;
  `_visible_prose_units`→`_VisibleTextParser` que dropa subárvores hidden/styled e separa `<br>`/bloco;
  `_claim_spans` whole-unit), binding **ref==runstore:address**, claim==unidade-de-prosa-inteira, e valor
  atestado == token numérico standalone (`_NUM_TOKEN`). +31 testes (`tests/test_internal_evidence_cite.py`).
- **Residual aceito (operador):** **R8-prov** — autenticação-de-runner / proveniência-de-medição fora de
  escopo (precisa de infra de assinatura ausente; honestidade-de-medição é do revisor cego + humano, ADR-0013).
  Documentado em requirements R8. Trilha de evolução: `docs/enrichment-evolution/goal3-slices/S3-SHIPPED/`.
...
### S4 — Discharge persiste entre rodadas (R9)  ✅ SHIPPED (codex approve)
- **Entregue em `tools/close.py`:** ledger de discharge no improve loop de `run_close` (`_norm_strike`/
  `_round_strikes`/`_discharge_verdict`). Um strike RESOLVIDO numa rodada (levantado, depois ausente após a
  revisão) é carimbado e sua RE-EMERGÊNCIA nos mesmos termos é suprimida → o loop CONVERGE em vez de rodar
  o alvo-que-recua até `IMPROVE_BACKSTOP` (livelock do roberto). **Discharge é LOOP-ONLY** (Codex S4 #1): o
  gate continua autoritativo (exige revisores atuais limpos em todo achado bloqueante não-genus), então um
  achado descarregado prematuramente por um falso-negativo é pego no gate e nunca emite proof. Genus é
  determinístico, nunca descarregado. +3 testes em `tests/test_close_loop.py::DischargePersistsAcrossRounds`.
  Discharge semântico no nível do gate/conductor = R5/S9. Trilha: `docs/enrichment-evolution/goal3-slices/S4-SHIPPED/`.
...
### S2 — O gate dominante R0 (storytelling + completude)  ✅ SHIPPED (codex approve, iter-22)
- **Entregue em `tools/close.py`:** `_check_storytelling_floor` (wired no `check_genus`) — R0-II(d)
  DETERMINÍSTICO: uma seção (ou metrics top-level) com estrutura visual/rotulada (palette visual + tabelas
  + list + card/numbered-card/risk-table/code-block/template-block) DEVE ter prosa reader-visível
  acompanhando. Visibilidade = visible-glyph + allowlist-de-propriedades-seguras + valores numéricos
  float-parseados, checados contra o style SANITIZADO que o render emite (`render.safe_style`). R0-I
  (adequação da definição) + R0-III (preservação de claim da fonte) = SEMÂNTICO no dimension
  `narrative_depth` (ADR-0013). **Diferido (documentado):** mecanização do claim-inventory tipado de R0-III
  + calibração das métricas narrativas do livro `eeb696e` (R0-II) = a slice de MIGRAÇÃO do livro, não o
  gate genérico. +33 testes `tests/test_storytelling_floor.py`. Trilha: `docs/enrichment-evolution/goal3-slices/S2-SHIPPED/`.
...
### S5 — A/B single-writer vs fan-out (R0a, decide a arquitetura)  ⏸ DEFERRED (runtime experiment)
- **Tratamento:** experimento, NÃO feature (explícito no requirements). O invariante R0a já é garantido
  pela S2 shipada (qualquer arquitetura que passe R0(II) satisfaz R0a). O A/B em si precisa do conductor
  LIVE (neo4j + venv + keys) sobre material idêntico — ação de operador, não mudança de working-tree.
  Protocolo + métricas + regra-de-decisão documentados em `docs/enrichment-evolution/goal3-slices/S5-experiment-deferred/`.
...
### S7 — Boundary de grounding de TODO visual claim-bearing (owed OU opcional) (R2/R3) — **ANTES do floor**  ⏸ DEFERRED (design + blockers documented)
...
### S6 — Floor capability-conditional, subordinado a R0 **E a S7** (R1)  ⏸ DEFERRED (design + blockers documented)
...
### S9 — Conductor: semantic-discharge CUT-com-razão (R5; aplica a severidade nascida em S1)  ✅ SHIPPED (codex approve)
