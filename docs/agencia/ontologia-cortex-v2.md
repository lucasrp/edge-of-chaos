...
Mais uma entidade de 1ª classe: **parceiro de trabalho** — empresa, pesquisador, membro da equipe, usuário de git ou o próprio operador/mentorado do agente. O roberto sabe de cor o pessoal do trabalho do operador; o grafo também deve saber. Props: `{name, kind: empresa|pesquisador|equipe|git-user|operador, domain?, contact_ref?}`; arestas: artefato-`PARA`->parceiro (o documento FEITO pra pessoa), parceiro-`MENTIONS`-tópicos/entities. `provenance_class`: asserted quando declarado, extracted quando minerado.
