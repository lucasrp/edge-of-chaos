...
- **R8 — Tier "internal-evidence" no gate de cite/grounding (bloqueante p/ Artefato de medição).** Dado
  ...
  **proveniência INTERNA DURÁVEL ANCORADA NO EVENTLOG** (ADR-0006: o eventlog é a **fonte única** de
  verdade) — um evento **append-only** com valores+hash, OU um store de run content-addressed
  **referenciado por um evento append-only que carrega o manifesto imutável** (p/ replay); **NÃO** blob/
  path de Artefato publicado (prunable), **nem** um store paralelo sem âncora no eventlog (seria 2ª
  autoridade) — em vez de cite externo, **apenas no gate de cite/grounding e apenas p/ claims numéricos
  internos**. **CISÃO obrigatória (gate reframe-3,
  ...
  *Limite de proveniência (RESIDUAL ACEITO — decisão de operador, S3 codex iter-9 [high]):* o
  content-addressing do runstore garante **integridade + binding** — um registro não resolve sob endereço
  diferente do seu conteúdo (anti-forja/anti-tamper de *alteração*), e o número é amarrado claim↔valor↔
  endereço↔prosa-visível. **NÃO** garante **autenticação-de-runner** (que a medição veio de um harness
  confiável): `record_run` aceita run_id/values de qualquer chamador no mesmo domínio de confiança. Fechar
  isso exige infra de **assinatura por chave do harness / proveniência de job** que **não existe** na
  arquitetura atual; e fabricar a *própria* medição é, por ADR-0013, questão do **revisor cego + humano**,
  não garantia do gate determinístico. Aceito como residual e rastreado como **R8-prov** (requisito
  futuro, fora do escopo de S3); o gate determinístico entrega integridade+binding, não honestidade-de-medição.
...
- **R2 — Grounding como invariante verificável no publish, não como contrato-de-origem (bloqueante).**
  ...
  fronteira precisa ser mecanicamente verificável **no caminho que publica**. Mecanismo: **proof-digest
  por-visual ligado ao proof do close (PROOF-ONLY, gate reframe-3 F-pub).** `add_visuals` (no produtor,
  com a evidência viva) **cunha uma ATTESTATION INFORJÁVEL por-visual APENAS quando `attributable()`
  passa** — um token **assinado** (HMAC com o segredo do passo de grounding/close que o produtor **não
  reproduz**) sobre {dados-do-bloco canônicos + ids-de-span-de-evidência}, incluído **separadamente** no
  proof assinado do close. O publish **re-verifica a attestation** (prova "passou pelo grounding", não só
  "payload íntegro") e strippa o visual sem attestation válida. **Um digest auto-consistente que o
  próprio produtor computa NÃO basta** (distingue provenance de integridade — gate reframe-4 F1).
  **Strip-and-reinsert no publish está FORA** (exigiria evidência/contexto no publish, violando o
  blindfold — ADR-0013: publish é blind, final-Artefato-only). `add_visuals` retorna **shortfall** quando
  a evidência não sustenta, nunca fabrica. *(Reusa `visuals.py`; resolve Q3; proof-only respeita ADR-0013.)*
  *Aceitação extra:* um visual **autorado direto** com digest **auto-consistente** e proof de close
  válido é **rejeitado** (falta a attestation do grounding).
...
- **Q3. (RESOLVIDA — attestation inforjável, proof-only.)** O mecanismo é uma **attestation por-visual
  assinada (HMAC com segredo do grounding/close que o produtor não reproduz), cunhada SÓ quando
  `attributable()` passa**, incluída separada no proof do close; o publish **re-verifica a attestation**
  (provenance "passou pelo grounding", não só integridade de digest) — proof-only, nenhuma evidência/
  contexto do produtor cruza (ADR-0013 blind publisher). **Inserção centralizada está FORA**. A fiação
  produtor→`add_visuals` (estender do conductor aos standalone) é necessária. Red tests:
  forjado/stale/mismatched, **visual autorado direto com digest auto-consistente + proof de close válido
  REJEITADO** (sem attestation do grounding), opcional não-sustentado, fake-decorativo-com-dado.
