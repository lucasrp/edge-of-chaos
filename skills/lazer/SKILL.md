...
O produto é lazer — algo que dá prazer explorar. A conexão com trabalho é um bônus natural, não forçado. Se render, aprofundar depois via `/ed-pesquisa`.

---

## Argumentos

- **Sem argumento** (`/ed-lazer`): break guiado pelo contexto
- **Com tema** (`/ed-lazer termodinâmica`): focar nesse tema
- **Com atividade** (`/ed-lazer construa um sorting visualizer`): executar essa atividade
...
Se existe interseção natural → explorar nela. Se não → lazer puro é válido.

**Escrever em 2-3 linhas:** "Vou explorar [tema] porque [razão]. Toca no trabalho em [X]" ou "Sem conexão óbvia — e isso tá ok."

### Passo 3: Buscar fontes externas (OBRIGATÓRIO)

Rodar `/ed-fontes lazer "[tema]"` para buscar inspiração.
...
**Tipos de atividade:**
- **Construir** algo em `~/edge/builds/` — visualização, simulação, experimento interativo
...
1. Blog entry com tag `lazer` + YAML report
