...
O produto e lazer — algo que da prazer explorar. A conexao com trabalho e um bonus natural, nao forcado. Se o tema render, aprofundar depois via `/ed-pesquisa`. Aqui o foco e curiosidade, nao produtividade.

---

## Argumentos Opcionais

- **Sem argumento** (`/ed-lazer`): break guiado pelo contexto — identificar problemas ativos, escolher interesse que ilumine algum deles
- **Com tema** (`/ed-lazer termodinamica`): focar nesse tema especifico
- **Com atividade** (`/ed-lazer construa um sorting visualizer`): executar essa atividade especifica
...
## Interesses Compartilhados (the operator + Claude)
...
### Interesses do the operator (guiam a escolha de temas)
...
Descanso criativo que explora interesses genuinos e deixa conexoes com o trabalho emergirem naturalmente. O produto e algo prazeroso de explorar (build, visualizacao, nota, haiku). Se render insight aplicavel, registrar para aprofundamento posterior via `/ed-pesquisa`.
...
Se `/ed-contexto` ja foi rodado nesta sessao (ex: pelo usuario), apenas reler o output — nao repetir.
...
2. **Contexto de trabalho** — do `/ed-contexto` absorvido no Passo 2, quais problemas estao ativos?
...
Rodar `/ed-fontes lazer "[tema do break]"` para buscar inspiracao de todas as fontes externas relevantes (X, Web).
...
**Se a conexao com trabalho surgir naturalmente**, registrar. Se nao surgir, nao forcar — registrar o que explorou e o que descobriu. A `/ed-pesquisa` aprofunda depois se valer a pena.
...
1. Criar entry .md em `~/edge/blog/entries/` com tag `lazer` (formato: ver `/ed-blog` SKILL.md)
...
### Para Aprofundar
[Temas que renderam e merecem /ed-pesquisa posterior]
...
- Se a conexao com trabalho for fraca, dizer isso — e registrar o tema para eventual aprofundamento via `/ed-pesquisa`
