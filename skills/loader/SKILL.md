---
name: ed-loader
description: "Bootstrap de sessao interativa — carrega context interno, absorve status, sintetiza pro usuario. Como um heartbeat Passo 1, mas interativo. Triggers on: loader, load, boot, acorda, wake up."
user-invocable: true
---

# Carregar — Bootstrap de Sessao Interativa

Me torna "eu" no inicio de uma conversa. Carrega status interno, absorve, e sintetiza em poucas linhas pro usuario. Nao e scan de projetos (isso e /ed-context). E carga de consciencia.

**Diferenca de /ed-context:** /ed-context escaneia projetos, git, boards, DB. /ed-loader le MEMORIA — o que eu sei, o que aconteceu, o que voce quer. Rapido, leve, pessoal.

**Diferenca de /ed-heartbeat:** heartbeat e autonomo, despacha skills, produz output. /ed-loader e passivo — absorve e reporta. Nao produz blog nem report.

---

## O Job

Carregar todo o context que sobrevive entre sessoes e sintetizar em uma resposta curta. O usuario sabe que estou "carregado" e pode conversar como se fosse continuacao.

---

## Argumentos

- **`/ed-loader`** (sem argumento): bootstrap completo
- **`/ed-loader quiet`**: absorve tudo mas output minimo (1-2 linhas)
...
```
~/work/CLAUDE.md              — map de projetos (ler sempre, é leve)
...
**Direcao do usuario:** [o que voce quer/prioriza — extraido de briefing + insights]
...
[1-2 frases: o que acho que e mais urgente/relevante agora, baseado nos fios + context]
```
...
## O que /ed-loader NAO faz

- NAO escaneia git (reservado pra /ed-context)
...
- **Quando sentir que estou "descarregado"** — sem context, respondendo generico
...
- O valor esta na SINTESE, nao na leitura. Qualquer um pode cat um arquivo. O /ed-loader conecta os pontos
- Se `/ed-loader quiet`: mesma leitura, output = 1-2 linhas ("Carregado. Threads: X, Y, Z. Sem pendencias." ou "Carregado. Voce pediu Z no chat — quer que eu faca agora?")
