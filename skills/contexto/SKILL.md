...
**O que /ed-contexto NAO e:**
- NAO e /ed-fontes (mundo externo — X, Web, ArXiv)
- NAO e /ed-mapa (conexoes entre ideias internas)
...
O /ed-contexto adapta a profundidade ao caller. Quem chama determina o que e lido.

### Como identificar o caller

- Se invocado com argumento explicito (`/ed-contexto estrategia`): usar o argumento
- Se invocado por outra skill (heartbeat, pesquisa, etc.): o caller e a skill que esta rodando
- Se invocado manual pelo usuario (`/ed-contexto`): usar perfil "full"
- `/ed-contexto force`: perfil "full" ignorando cache
...
- Se /ed-contexto ja rodou nesta sessao (por qualquer caller), reutilizar o output anterior. Nao re-rodar
...
**Forcar refresh:** `/ed-contexto force` ignora cache e roda tudo.

---

### Tier 1 — CORE (fontes 1-4)

#### 1. Briefing (estado compilado)
...
grep -c '\[PENDENTE\]' ~/.claude/projects/$MEMORY_PROJECT_DIR/memory/descobertas.md 2>/dev/null || echo "0"

echo "=== Reflexao log ==="
grep -c '## \[' ~/.claude/projects/$MEMORY_PROJECT_DIR/memory/reflexao-log.md 2>/dev/null || echo "0 reflexoes"
```
...
- **Antes de /ed-estrategia** — obrigatorio (estrategia chama contexto)
- **Via /ed-heartbeat** — automaticamente
- **Quando pedido** — `/ed-contexto` ou `/ed-contexto [caller]`
...
**Relacao com /ed-fontes e /ed-mapa:**
- /ed-contexto = estado interno (onde estamos)
- /ed-fontes = sinais externos (o que ha de novo la fora)
- /ed-mapa = conexoes internas (como nossas ideias se ligam) — proposto, nao implementado
