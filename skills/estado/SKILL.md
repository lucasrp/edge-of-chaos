...
```bash
echo "=== DESCOBERTAS ==="
file="$HOME/.claude/projects/$MEMORY_PROJECT_DIR/memory/descobertas.md"
if [ -f "$file" ]; then
...
- `/ed-estado` — snapshot completo (default)
- `/ed-estado propostas` — apenas secao de propostas
- `/ed-estado saude` — apenas state files + anomalias (rapido)

---

## Quando Usar

- **Standalone:** `/ed-estado` — quando quiser um dashboard factual
- **Antes de /ed-heartbeat:** para informar o dispatch com dados concretos
- **Apos /ed-reflexao:** para verificar se o estado foi atualizado corretamente
- **Debug:** quando algo parece inconsistente — `/ed-estado saude`

---

## O que /ed-estado NAO faz

- NAO interpreta (isso e /ed-contexto)
- NAO recomenda acoes (isso e /ed-estrategia)
- NAO modifica nenhum arquivo (leitura pura)
- NAO le sessoes CLI ou logs de conversa (isso e /ed-contexto)
