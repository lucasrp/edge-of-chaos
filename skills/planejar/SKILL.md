...
- **Sem argumento** (`/ed-planejar`): analisar contexto e propor autonomamente
- **Com tema** (`/ed-planejar eval de prompts`): propor ciclo sobre esse tema
- **Com projeto** (`/ed-planejar my-project`): propor ciclo para esse projeto existente
- **Status** (`/ed-planejar status`): listar todas as propostas e seus estados

Exemplos:
- `/ed-planejar` → analisa contexto, propoe algo relevante
- `/ed-planejar blog dashboard` → propoe ciclo para nova feature no blog
- `/ed-planejar backend` → propoe ciclo para o backend existente
- `/ed-planejar status` → dashboard de propostas
...
cat ~/.claude/projects/$MEMORY_PROJECT_DIR/memory/descobertas.md 2>/dev/null
...
Usar `ultrathink` (thinkmax). Com o output do `/ed-contexto` + as fontes acima, identificar:
...
Rodar `/ed-fontes planejar "[tema da proposta]"` para obter experiencias praticas de implementacao de todas as fontes relevantes (Web, X, GitHub, HN).
...
Consultar com `/ed-planejar status`.
```
...
- **Via /ed-heartbeat:** Quando contexto sugere oportunidade de projeto
- **Manualmente:** `/ed-planejar` — "proponha um ciclo de desenvolvimento"
- **Com direcao:** `/ed-planejar eval system` — "proponha ciclo sobre isso"
- **Status:** `/ed-planejar status` — "quais propostas existem?"
- **Apos /ed-pesquisa:** Quando pesquisa produziu recomendacoes que merecem proposta detalhada
