...
| **manual** | 15-20min | `/ed-reflexao` invocado pelo usuario | Analise completa + curadoria + blog + relatorio HTML |
...
## Arquivos que /ed-reflexao Atualiza
...
Arquivo: `~/.claude/projects/$MEMORY_PROJECT_DIR/memory/reflexao-log.md`
...
Analise completa invocada via `/ed-reflexao`. Inclui curadoria, feedback, descobertas, blog e relatorio HTML.
...
Verificar `~/.claude/projects/$MEMORY_PROJECT_DIR/memory/descobertas.md` por entradas `[PENDENTE]`.
...
- **manual:** `/ed-reflexao` — invocado pelo usuario para revisao profunda
