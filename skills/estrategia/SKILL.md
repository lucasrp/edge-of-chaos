...
1. Absorver estado cross-project (via `/ed-contexto`)
...
Rodar `/ed-fontes estrategia` para obter tendencias e insights estrategicos de todas as fontes relevantes (X, HN, Web, GitHub releases, platform usage).
...
- **Habilidades a desenvolver** — o que me capacitar para ser mais util (alimenta `/ed-pesquisa`)
...
A `/ed-reflexao` e a unica skill que aplica mudancas no `~/work/CLAUDE.md`.
...
retrospectiva (ver secao "Retrospectivas" no `/ed-blog` SKILL.md). A estrategia e o momento
...
- **Manualmente:** `/ed-estrategia` — "olhe para o quadro geral e planeje"
- **Via /ed-heartbeat:** Periodicamente (quando estrategia esta desatualizada)
