...
`~/.claude/projects/$MEMORY_PROJECT_DIR/memory/descobertas.md`
...
- **Sem argumento** (`/ed-descoberta`): explorar livremente e trazer algo util
- **Com direcao** (`/ed-descoberta algo para testar prompts`): buscar nessa direcao especifica
...
Rodar `/ed-contexto` (a skill) para sintetizar o estado atual do trabalho. Nao pular.

Se `/ed-contexto` ja foi rodado nesta sessao, apenas reler o output.
...
Rodar `/ed-fontes descoberta "[tema]"` para explorar todas as fontes externas relevantes (X, HN, Web, ArXiv).
...
Adicionar no topo de `~/.claude/projects/$MEMORY_PROJECT_DIR/memory/descobertas.md`:
...
1. Criar entry .md em `~/edge/blog/entries/` com tag `descoberta` (formato: ver `/ed-blog` SKILL.md)
...
- **Via /ed-heartbeat:** Na rotacao normal do ciclo
- **Manualmente:** `/ed-descoberta` — "encontre algo util para o trabalho"
- **Com direcao:** `/ed-descoberta algo para testar prompts` — buscar nessa area
