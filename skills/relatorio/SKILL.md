...
# /ed-relatorio — Pensar Produzindo
...
Rodar `/ed-fontes relatorio "[tema central]"` para busca abrangente em TODAS as fontes externas (X, Web, ArXiv, HN, GitHub).
...
Montar o YAML spec com secoes e block types. O formato e o mesmo do `/relatorio`.
...
**4a. Blog interno:**
1. Criar entry .md com tag `relatorio` (ou da skill chamadora). Formato: ver `/ed-blog` SKILL.md
...
- Ou adicionar como entrada em `~/.claude/projects/$MEMORY_PROJECT_DIR/memory/descobertas.md` com `[PENDENTE]`
- A `/ed-reflexao` vai processar na proxima execucao
