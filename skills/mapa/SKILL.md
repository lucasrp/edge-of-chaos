...
**O que /ed-mapa NAO e:**
- NAO e /ed-contexto (estado atual) — mapa e sobre CONEXOES, nao estado
- NAO e /ed-fontes (mundo externo) — mapa e sobre o que JA SABEMOS internamente
...
- "Quais descobertas se aplicam ao projeto X?"
...
- **`/ed-mapa [conceito]`**: mostrar todas as conexoes de um conceito especifico
- **`/ed-mapa [A] [B]`**: mostrar caminho de conexao entre A e B
- **`/ed-mapa full`**: mapa completo (pesado, so para estrategia)
...
| `~/.claude/projects/$MEMORY_PROJECT_DIR/memory/descobertas.md` | Descobertas ↔ projetos | Read direto |
...
A busca semantica e o motor principal do /ed-mapa. Encontra conexoes que compartilham conceito sem compartilhar keywords.
...
- /ed-mapa e leve. Nao carrega contexto pesado. Busca direcionada
- Pode ser chamado por outras skills (/ed-estrategia, /ed-planejar) como consulta auxiliar
