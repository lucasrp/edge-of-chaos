...
def _read_doc(name):
    ...
    md, tpl = MEMORY / f"{name}.md", MEMORY / f"{name}.md.tpl"
    if md.exists():
        return md.read_text().strip()
    if tpl.exists():
        return _render_tpl(tpl.read_text()).strip()
    return ""


def _section_tattoos():
    ...
    historical). Degrades to a marker if a doc is absent; never crashes."""
    personality = _read_doc("personality")
    method = _read_doc("method")
    if not personality and not method:
        return ("## Initial tattoos (immutable) — personality & method\n\n"
                "_no doctrine inscribed yet._")
    parts = ["## Initial tattoos (immutable) — who I am, how I think",
