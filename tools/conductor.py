...
def _parse_decline(v: dict):
    """A REVIEWER-bound accepted decline (R5/S9), or None. The judge — not the producer — accepts a
    dropped finding as NON-BLOCKING by returning `decline: {accepted: true, severity: "non_blocking",
    reason: "..."}`. Anything else (no decline, not accepted, severity not non_blocking, blank reason) is
    None → the drop stays blocking. This binds the acceptance to the semantic judge's output, so a producer
    cannot self-attest away a correctness drop."""
    d = v.get("decline")
    if (isinstance(d, dict) and d.get("accepted") is True and d.get("severity") == "non_blocking"
            and isinstance(d.get("reason"), str) and d["reason"].strip()):
        return {"accepted": True, "severity": "non_blocking", "reason": d["reason"].strip()}
    return None


def _parse_discharge(raw: str, expected_ids: list[str]) -> dict:
    """Tolerant parse of the semantic judge's verdicts into {finding_id: {delivered: bool, decline: dict|
    None}}. Mirrors excavate._parse_seed: strip a fence, drop malformed verdicts, NEVER crash. A finding
    the judge failed to rule on (or a garbled response) defaults to NOT delivered + NO accepted decline
    (fail-closed) — a node cannot mint a discharge OR a decline from a refusal."""
    by_id = {fid: {"delivered": False, "decline": None} for fid in expected_ids}
    if not raw or not raw.strip():
        return by_id
    text = raw.strip()
    fence = re.search(r"```(?:json)?\s*(\{.*?\})\s*```", text, re.DOTALL | re.IGNORECASE)
    ...
    for v in data.get("verdicts") or []:
        ...
        if fid in by_id:
            by_id[fid] = {"delivered": v.get("delivered") is True,  # exact identity, no coercion
                          "decline": _parse_decline(v)}
    return by_id
...
def semantic_discharge(node: dict, seed: dict, complete_fn) -> list[dict]:
    ...
    return [{"finding_id": fid, "delivered": by_id[fid]["delivered"],
             "decline": by_id[fid]["decline"]} for fid, _ in findings]
...
def _grounding_blocks(visual_flags) -> list[str]:
    """Grounding (R2) failures in the visual post-pass — a shortfall or a dropped spot means a visual's
    data was not attributable to the evidence. These FAIL CLOSED (never a logged residual)."""
    return [f"grounding:{f}" for f in (visual_flags or [])
            if "shortfall" in str(f).lower() or "dropped spot" in str(f).lower()]


def circuit_breaker(result: dict, *, render_ok=True) -> dict:
    """R5 (S9) — the conductor's CUT-with-reason + severity-classified disjuntor. Partition the run's
    unresolved findings/violations into BLOCKING vs NON-BLOCKING and decide ship:

      - **Fail-closed (blocking, NEVER a residual):** any gate/genus violation, a synthetic-shape
        violation, a GROUNDING shortfall (R2), a render failure, AND a DROPPED assigned finding
        (delivered:false) the SEMANTIC JUDGE did not accept as a non-blocking decline.
      - **Logged residual (non-blocking, ships):** a dropped finding the judge ACCEPTED as a non-blocking
        decline; plus non-blocking quality flags (form/opener) which are logged, not failed.

    The decline acceptance is REVIEWER-BOUND (Codex S9): it is read from the per-finding semantic-discharge
    records in `result['outline'][*]['discharge']` — the injected semantic judge's output, NOT a
    caller/producer parameter — so a producer cannot self-attest away a correctness drop. A decline counts
    only when the judge's record carries `decline={accepted:True, severity:'non_blocking', reason:…}`
    (parsed fail-closed in `_parse_decline`). Structural failures (grounding/genus/render/shape) are never
    declinable. Ships (`status='final'`) iff nothing blocking remains; else `status='blocked'` — fail-closed
    even at the round cap. Mirrors close's S1 severity at the conductor altitude; combines with R7 (cap) and
    R9 (discharge). Returns {ship, status, residuals: [...], blocking: [...]}."""
    blocking, residuals = [], []
    genus = result.get("genus") or {}
    for spec in ("deep", "synthetic"):
        blocking += [f"genus:{spec}:{v}" for v in (genus.get(spec) or [])]
    blocking += [f"synthetic-shape:{v}" for v in (result.get("synthetic_shape") or [])]
    blocking += _grounding_blocks(result.get("visual_flags"))
    if not render_ok:
        blocking.append("render:failed")
    # per-node DETERMINISTIC contract-gate failures (empty node, a stripped assigned finding, …) are
    # blocking and NEVER declinable — a clean/accepted semantic discharge can't clear a deterministic gate.
    for node in (result.get("outline") or []):
        for g in (node.get("gate") or []):
            blocking.append(f"gate:{node.get('id')}:{g}")
    # dropped findings — classified by the JUDGE's reviewer-bound discharge record, never a caller param.
    for node in (result.get("outline") or []):
        for v in (node.get("discharge") or []):
            if not isinstance(v, dict) or v.get("delivered") is True:
                continue
            fid = v.get("finding_id")
            decline = v.get("decline")
            if isinstance(decline, dict) and decline.get("accepted") is True \
                    and decline.get("severity") == "non_blocking" \
                    and isinstance(decline.get("reason"), str) and decline["reason"].strip():
                residuals.append({"finding_id": fid, "reason": decline["reason"]})  # judge-accepted decline
            else:
                blocking.append(f"dropped-finding:{fid}")    # not delivered, no judge-accepted decline
    residuals += [{"flag": f} for f in (result.get("form_flags") or [])]
    residuals += [{"flag": f} for f in (result.get("opener_flags") or [])]
    ship = not blocking
    return {"ship": ship, "status": "final" if ship else "blocked",
            "residuals": residuals, "blocking": blocking}
...
def run_conductor(seed: dict, objective: str, complete_fn, *, is_enabled=None,
    ...
    if not on:
        return {"enabled": False, "passthrough": True, "content": None, "outline": [],
                ...
                "diversity": diversity_report({"sections": []}),
                # R5 (S9): neutral circuit-breaker verdict when off — nothing produced, nothing to block.
                "ship": True, "status": "final", "residuals": [], "blocking": []}
    ...
    result = {"enabled": True, "passthrough": False, "content": deep_spec, "outline": final_nodes,
    ...
    # R5 (S9): the conductor's OWN severity circuit-breaker decides ship at the conductor altitude — a
    # blocking finding (grounding/genus/shape/render/judge-undelivered-drop) fails CLOSED here, never
    # silently surfaced for a caller to miss. Decline acceptance is read from the per-node JUDGE discharge
    # records (reviewer-bound), never a caller param. Merged into the returned result.
    _cb = circuit_breaker(result)
    result.update(ship=_cb["ship"], status=_cb["status"],
                  residuals=_cb["residuals"], blocking=_cb["blocking"])
    return {**result,
