...
def compose_semantic_brief(query, hits=_AUTO, *, group=None, limit=SEMANTIC_SEARCH_LIMIT,
    ...
    Dark hits → honest marker, never a crash.
    """
    if hits is _AUTO:
        hits = semantic_search(query, group=group, limit=limit, embed_fn=embed_fn)
    ...
    if hits is None:
        return (SEMANTIC_BANNER + "\n# Semantic search — common graph entry\n\n"
                ...
                "_Semantic leg DARK (no embedder, no corpus, or empty query) — fall back to "
                "space-0 push (`compose_recall_brief`) or structural cypher "
                "(`skills/_shared/memory.md`)._\n")
    lines = [
        SEMANTIC_BANNER,
        "# Semantic search — common graph entry",
        f"_Query:_ {q}",
