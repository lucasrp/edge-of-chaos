...
def install_hermes_plugin(cfg, repo, edge_home, hermes_root):
    ...
    (plugin / "__init__.py").write_text(
        ...
        "    briefing = result.stdout[briefing_at:recall_at if recall_at >= 0 else next_section if next_section >= 0 else None]\n"
        "    dispatch_id = next((line for line in result.stdout.splitlines() if line.startswith(\'DISPATCH_ID=\')), \'\')\n"
        "    direction = (EDGE_HOME / \'state\' / \'direction.md\').read_text() if (EDGE_HOME / \'state\' / \'direction.md\').is_file() else \'\'\n"
        "    cache = EDGE_HOME / \'state\' / \'live\' / \'hermes-preflight\' / (session_id + \'-producer-\' + matched + \'.md\')\n"
        "    cache.parent.mkdir(parents=True, exist_ok=True)\n"
        "    cache.write_text(dispatch_id + \'\\n\' + briefing + \'\\n\' + direction)\n"
        "    return {\'context\': \'EOC PRODUCER PREFLIGHT — MECHANICAL FLOOR COMPLETE. You are producing an artefato for skill `\' + matched + \'`. The canonical driver already ran predispatch (assemble drain, readiness gate, sweep, briefing composition, dispatch stamp, grounding, recall). Read the full briefing from \' + str(cache) + \'. Follow the shared scaffold: S1 Grounding, S2 Research, S3 Build the artefato spec, S4 Close (call close.run_close with slug=`\' + matched + \'-<brief-slug>`, skill=`\' + matched + \'`, spec=<your-spec>, intent=<your-intent>, cites=<your-cites>, dispatch_id=`\' + dispatch_id + \'`), S5 Publish (call publisher.publish with the close_proof). After publish, emit source.signal for every cited snippet. If close or publisher tools are not available in this environment, produce the spec + intent + cites as structured output and document what would be published. DISPATCH=\' + dispatch_id}\n"
        "@_release_lease_on_error\n"
