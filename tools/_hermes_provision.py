...
def install_hermes_plugin(cfg, repo, edge_home, hermes_root):
    ...
    (plugin / "__init__.py").write_text(
        ...
        "    # Inspect only the current/last user turn so an older skill invocation\n"
        "    # cannot retrigger a preflight on a later message.\n"
        ...
        "    record = {'ts': time.time(), 'session_id': session_id, 'matched': matched or '', 'user_len': len(user_message or ''), 'history_count': len(conversation_history or []), 'current_user_len': len(current), 'user_has_important': '[IMPORTANT:' in (user_message or ''), 'current_has_important': '[IMPORTANT:' in current}\n"
