...
def install_hermes_plugin(cfg, repo, edge_home, hermes_root):
    ...
    (plugin / "__init__.py").write_text(
        ...
        "        return '\\n'.join(str(part.get('text', '')) for part in content if isinstance(part, dict) and part.get('type') in {'text', 'input_text'})\n"
        "    return ''\n"
        ...
        "        return True\n"
        "    return False\n"
        "def _current_user_text(user_message, conversation_history):\n"
        "    original = _content_text(user_message).strip()\n"
        "    for message in reversed(conversation_history or []):\n"
        "        if not isinstance(message, dict) or message.get('role') != 'user':\n"
        "            continue\n"
        "        current = _content_text(message.get('content', ''))\n"
        "        # The expanded current turn preserves the original request text. If it\n"
        "        # does not, history has not appended this turn yet; never reuse an older\n"
        "        # skill invocation as the current one.\n"
        "        if original and original not in current:\n"
        "            return ''\n"
        "        return current\n"
        "    return ''\n"
        "def _current_turn_invokes(user_message, conversation_history, slug):\n"
        "    # Hermes invokes pre_llm_call with original_user_message (which may omit\n"
        "    # the slash-skill marker) plus conversation_history after skill expansion.\n"
        "    return _invokes(user_message, slug) or _invokes(_current_user_text(user_message, conversation_history), slug)\n"
        "def _trace_preflight(session_id, user_message, conversation_history, matched):\n"
        "    import hashlib, json, os, time\n"
        "    user_text = _content_text(user_message)\n"
        "    current = _current_user_text(user_message, conversation_history)\n"
        "    trace = EDGE_HOME / 'state' / 'live' / 'hermes-preflight' / 'hook-trace.jsonl'\n"
        "    trace.parent.mkdir(parents=True, exist_ok=True)\n"
        "    if trace.exists() and trace.stat().st_size > 262144:\n"
        "        trace.unlink()\n"
        "    record = {'ts': time.time(), 'session': hashlib.sha256(str(session_id).encode()).hexdigest()[:12], 'matched': matched or '', 'user_len': len(user_text), 'history_count': len(conversation_history or []), 'current_user_len': len(current), 'user_has_important': '[IMPORTANT:' in user_text, 'current_has_important': '[IMPORTANT:' in current}\n"
        "    fd = os.open(trace, os.O_APPEND | os.O_CREAT | os.O_WRONLY, 0o600)\n"
        ...
        "def mentor_preflight(user_message='', session_id='', conversation_history=None, **_):\n"
        "    if not _current_turn_invokes(user_message, conversation_history, 'mentor'):\n"
        "        return None\n"
        ...
        "@_release_lease_on_error\n"
        "def assemble_preflight(user_message='', session_id='', conversation_history=None, **_):\n"
        "    if not _current_turn_invokes(user_message, conversation_history, 'assemble'):\n"
        "        return None\n"
        "    import subprocess\n"
        ...
        "@_release_lease_on_error\n"
        "def delta_preflight(user_message='', session_id='', conversation_history=None, **_):\n"
        "    if not _current_turn_invokes(user_message, conversation_history, 'delta'):\n"
        "        return None\n"
        "    import json\n"
        ...
        "@_release_lease_on_error\n"
        "def dig_preflight(user_message='', session_id='', conversation_history=None, **_):\n"
        "    if not _current_turn_invokes(user_message, conversation_history, 'dig'):\n"
        "        return None\n"
        ...
        "@_release_lease_on_error\n"
        "def recall_preflight(user_message='', session_id='', conversation_history=None, **_):\n"
        "    if not _current_turn_invokes(user_message, conversation_history, 'recall'):\n"
        "        return None\n"
        "    import os, subprocess\n"
        "    env = _edge_env(session_id)\n"
        ...
        "@_release_lease_on_error\n"
        "def producer_preflight(user_message='', session_id='', conversation_history=None, **_):\n"
        "    import subprocess\n"
        "    PRODUCERS = {'report', 'research', 'map', 'plan', 'discovery', 'prototype', 'lazer', 'critique'}\n"
        "    matched = next((s for s in PRODUCERS if _current_turn_invokes(user_message, conversation_history, s)), None)\n"
        "    _trace_preflight(session_id, user_message, conversation_history, matched)\n"
        "    if not matched:\n"
        "        return None\n"
        "    group = _active_edge_group()\n"
        "    env = _edge_env(session_id)\n"
