...
_EXPECTED_MEMORY_PROJECT_DIR="${EDGE_REPO_DIR//\//-}"
_LEGACY_MEMORY_PROJECT_DIR="${_EXPECTED_MEMORY_PROJECT_DIR#-}"
if [ -n "${MEMORY_PROJECT_DIR:-}" ] && [ "$MEMORY_PROJECT_DIR" = "$_LEGACY_MEMORY_PROJECT_DIR" ]; then
  MEMORY_PROJECT_DIR="$_EXPECTED_MEMORY_PROJECT_DIR"
fi
...
export \
  ...
  BUILDS_DIR HEALTH_DIR SNAPSHOT_DIR SCRATCHPADS_DIR STATE_DIR STATE_EVENTS_DIR PROJECTIONS_DIR AUDITS_DIR SIGNALS_DIR EXPLORATIONS_DIR \
  RUNTIME_PROTOCOL_DIR \
  SEARCH_STATE_DIR DB_DIR SEARCH_DB_FILE BLOG_FTS_DB_FILE BRIEFING_FILE EVENTS_FILE GIT_SIGNALS_FILE \
  CURRENT_BEAT_FILE CURRENT_DISPATCH_FILE TASKS_SNAPSHOT_FILE CURADORIA_CANDIDATES_FILE CURATION_DIGEST_FILE PROPOSALS_FILE PROCEDURE_CURATION_FILE WORKFLOW_HEALTH_FILE FIRST_STEPS_FILE DISPATCH_QUEUE_FILE BEAT_CONTEXT_FILE HEARTBEAT_ROTATION_FILE SOURCE_USAGE_FILE SOURCES_MANIFEST_FILE SOURCE_AFFORDANCE_DIGEST_FILE PRIMITIVES_STATUS_FILE CAPABILITIES_STATUS_FILE ASSET_INVENTORY_FILE RENDER_INSTALL_DRIFT_FILE DISPATCH_CYCLES_FILE PIPELINE_STATE_FILE OPERATOR_PRESSURE_PROJECTION_FILE OPEN_GAPS_DIGEST_FILE CONTINUITY_DELTAS_DIR PREFLIGHT_LOG_FILE OPS_HOTSPOTS \
